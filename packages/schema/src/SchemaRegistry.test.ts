...
beforeEach(() => {
  ajv = freshAjv()
  registry = new SchemaRegistry(ajv)
})
...
describe('compile validation', () => {
  test('compile() returns working validator with $ref resolution', () => {
    const coreId = makeCoreId()
    const schema: JSONSchema7 = {
      type: 'object',
      properties: { name: coreId },
  ...
  test('compile() rejects invalid data through nested $ref', () => {
    const coreId = makeCoreId()
    const coreRev = makeCoreRev()
    const coreLink = makeCoreLink(coreId, coreRev)
  ...
  test('compile() works with combinator $ref', () => {
    const coreRev = makeCoreRev()
    const schema: JSONSchema7 = {
      type: 'object',
      properties: {
  ...
  test('compile() works with additionalProperties $ref', () => {
    const coreId = makeCoreId()
    const schema: JSONSchema7 = {
      type: 'object',
      additionalProperties: coreId,
...
function expectEnvelope(
  ...
  const envelope: Int64Envelope = registry.getInt64Paths(schema)
