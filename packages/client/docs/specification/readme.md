...
| 14  | [DevTools](sections/devtools.md)                           | Chrome extension plan for inspecting commands, events, cache, read models, sync, and storage.                                                                         |
| 15  | [EntityRef](sections/entity_ref.md)                        | Client-side entity lifecycle tracking via `EntityRef` — making pending temp IDs visible in read model data for automatic cache key and dependency wiring.             |
| 17  | [Aggregate Config](sections/aggregate_config.md)           | First-class aggregate configuration — decouples stream ownership from collections and enables explicit ID reconciliation.                                             |
