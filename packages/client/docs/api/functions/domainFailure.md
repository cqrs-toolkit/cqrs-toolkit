[**@cqrs-toolkit/client**](../README.md)

---

[@cqrs-toolkit/client](../globals.md) / domainFailure

# Function: domainFailure()

> **domainFailure**(`errors`): [`DomainExecutionResult`](../type-aliases/DomainExecutionResult.md)\<`never`\>

Defined in: [packages/client/src/types/domain.ts:101](https://github.com/Swifttt-Dev/cqrs-toolkit/blob/93be80a21907f07a104ca0e358c4b366dbf08b7d/packages/client/src/types/domain.ts#L101)

Helper to create a failed domain execution result.

## Parameters

### errors

[`ValidationError`](../interfaces/ValidationError.md)[]

## Returns

[`DomainExecutionResult`](../type-aliases/DomainExecutionResult.md)\<`never`\>
