...
test('recovers missed events after server WS outage', async ({ page, request }) => {
  ...
  const createRes = await request.post(`${API}/notes`, {
    data: { notebookId, title: 'Original', body: '' },
  })
  ...
  const updateRes = await request.post(`${API}/notes/${noteId}/command`, {
    data: { type: 'updateTitle', data: { title: 'Recovered' }, revision: revAfterCreate },
  })
  ...
  const bodyRes = await request.post(`${API}/notes/${noteId}/command`, {
    data: { type: 'updateBody', data: { body: 'some body' }, revision: revAfterTitle },
  })
  ...
  const secondRes = await request.post(`${API}/notes`, {
    data: { notebookId, title: 'After-Gap', body: '' },
  })
