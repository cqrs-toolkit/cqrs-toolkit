...
export class TodoAggregate extends AggregateRoot<TodoServerEvent> {
  ...
  create(payload: { content: string }, id: string, metadata: EventMetadata): void {
    const now = new Date().toISOString()
    this.applyChange(
      createEvent<TodoServerEvent>({
        type: 'TodoCreated',
        data: { id, content: payload.content, status: 'pending', createdAt: now },
        metadata,
      }),
    )
  }

  updateContent(payload: { content: string }, metadata: EventMetadata): void {
    this.applyChange(
      createEvent<TodoServerEvent>({
        type: 'TodoContentUpdated',
        data: { id: this._id, content: payload.content, updatedAt: new Date().toISOString() },
        metadata,
      }),
    )
  }

  changeStatus(payload: { status: TodoStatus }, metadata: EventMetadata): void {
    this.applyChange(
      createEvent<TodoServerEvent>({
        type: 'TodoStatusChanged',
        data: { id: this._id, status: payload.status, updatedAt: new Date().toISOString() },
        metadata,
      }),
    )
  }
