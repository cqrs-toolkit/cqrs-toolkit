...
Promotion is *deliberate, not speculative*.
...
If the content is "this might generalise someday" rather than "this clearly applies at the higher scope already" — wait.
...
|---|---|---|
| `decisions/` | Write a *new* ADR at the higher scope; cite the original; back-pointer in the original. The original stays in place. | ADRs are immutable. |
...
3. **Edit the moved file for the new scope.**
   ...
   - Generalise examples that were project-specific.
...
5. **Append a changelog entry** to the originating project's `evolution/_overview.md` *and* the destination's `evolution/_overview.md` (typically the repo-level one).
...
ADRs are immutable.
Promotion produces a *new* ADR at the higher level rather than moving the existing one.
...
2. **Add a back-pointer** to the original ADR.
   ...
   ```markdown
   **Generalised by ADR-NNNN (YYYY-MM-DD)** at `/docs/decisions/NNNN-foo.md`.
   ```
...
4. **Append a changelog entry** to the originating project's `evolution/_overview.md` *and* the repo-level `evolution/_overview.md`.
...
- **Immutable case:** the new higher-level ADR cannot be deleted (ADRs are immutable).
  ...
  The original project ADR's "Generalised by..." pointer stays as a historical breadcrumb to the superseded artifact.
