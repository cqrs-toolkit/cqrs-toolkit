...
| `explorations/` | design alternatives under evaluation; open questions and candidate approaches not yet committed *(no repo-level entries yet; client has its own explorations wing)* |
| [`decisions/`](decisions/_overview.md) | repo-wide ADRs — *why* specific cross-cutting choices were made (immutable) |
...
| `mindset/` | how to think about problems here (reasoning) — analytical framings *(no repo-level entries yet; client has its own mindset wing)* |
| [`projects/`](projects/_overview.md) | nested project castles — packages and demos as recursive entries |

The construction / procedure / reasoning triplet keeps the three "how" wings distinct: a pattern is *how to act*; a playbook is *how to do a procedure*; a mindset is *how to think*.
...
- **ADR immutability.**
  ...
  If circumstances change, write a new ADR that supersedes the old one and add a "Superseded by ADR-NNNN (YYYY-MM-DD)" back-pointer to the original.
  That back-pointer is the only legitimate post-acceptance edit.
  Generalisation to a higher scope produces a *new* ADR at that scope; the original gets a "Generalised by ADR-NNNN" back-pointer.
...
- **Two-file pattern, with scaling.**
  Anywhere both *current state* and *history of changes* matter, use paired files.
  ...
  The log explains *why* the change was made; git history covers what changed line-by-line.
  ADRs are the deliberate exception — immutable, supersession recorded in-file, no paired log ever.
...
| Refactoring or making architectural changes | *All* relevant ADRs in the project's `decisions/` — not just recent. Old decisions are exactly where forgotten constraints live. Plus top-level [`decisions/`](decisions/_overview.md) for cross-cutting impact, and any patterns the refactor will touch. |
...
When the row doesn't fit cleanly, descend toward the wing that *describes* the kind of question being asked — `intent/` for "what should this do," `explorations/` for "what might we do," `decisions/` for "why is it this way," `evolution/` for "when did this change," `patterns/` for "how do we do X here," `mindset/` for "how do I think about this class of problem," `playbooks/` for "what's the procedure for X."
...
| Discover a recurring activity worth codifying (on the *second* occurrence, not the first) | Playbook in the right-scoped `playbooks/` wing. |
...
- **Wings that don't:** `intent/` and `evolution/` are scoped to the level they describe — they coexist across levels rather than promote.
...
- **Don't create empty wings preemptively.**
  ...
  Filing a `_overview.md` in a project is enough; `intent/`, `explorations/`, `decisions/`, `evolution/`, `patterns/`, `playbooks/`, `mindset/` get created when there's content to put in them.
...
- **Never edit accepted ADRs** except to add a "Superseded by ADR-NNNN (YYYY-MM-DD)" back-pointer.
  If circumstances change, write a new ADR that supersedes the old one.
- **Don't migrate the locally-excluded working dirs into the castle without going through the archeological pass.**
  Those paths (`_active-plans/`, `_docs/`, `_gaps/`, `_plans/`, `_inactive-plans*/`, `_open-api/`, plus a few legacy files in `.git/info/exclude`) trace current and historical work; bulk-importing them without judgment corrupts the castle.
...
A subsequent post-setup archeological pass will reverse-engineer retroactive ADRs and evolution entries from the locally-excluded working dirs (`_active-plans/`, `_docs/`, `_gaps/`, `_plans/`, `_inactive-plans*/`, plus a few legacy files in `.git/info/exclude`).
Until that pass runs, those paths remain in place untouched as archeological sources.
