...
These look superficially redundant ("we apply the events at submit *and* in the pipeline?") and the natural reaction during a cleanup-timing redesign is to fold the two together: have the pipeline be the single applier, with submit-time just enqueueing.
...
- **Submit-time application** must happen immediately and synchronously with `client.submit()` so the UI shows predicted state without waiting for a pipeline drain.
  The user typed something, hit save, and expects the optimistic update *now* — not after the next pipeline tick, not after the next event arrives, *now*.
...
- Bugs in either flow can produce incorrect optimistic state in different ways: submit-time bugs show as immediate UI errors; pipeline-time bugs show as eventual-consistency drift.
  Both must be diagnosed in their own terms.
...
- Bugs in cleanup timing or pipeline dedup are *not* justifications for collapsing the two.
