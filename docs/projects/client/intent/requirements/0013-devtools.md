...
Cache keys are opaque UUIDs (per [`§3.2.1`](0003-cache-manager.md#321-cache-keys-and-cache-scopes)). Collection and params cannot be recovered from the key alone. The `cache:key-added` event's `CacheKeyIdentity` payload provides the reverse-lookup data the devtools needs — no separate event required.
