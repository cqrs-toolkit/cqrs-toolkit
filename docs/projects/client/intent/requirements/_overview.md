...
| 0016 | [Custom Collations](0016-custom-collations.md)         | Consumer-registered locale-aware comparators threaded through WASM SQLite and the JS-side sort in Mode A, with an `unsupportedCollations: 'error' \| 'degrade'` policy for backends that can't register comparators. |
