...
- **vs `intent/requirements/`**: requirements describe *what the project commits to building*; explorations describe *what we're considering*. An exploration that the project commits to becomes a requirement.
...
- **vs `mindset/`**: mindset is how to *think* about a class of problems (analytical framing); explorations are *what to do* about a specific open question (candidate answers under evaluation).
...
- **Generalised into a normative convention** → pattern entry in `patterns/`. Same `git mv` pattern when consuming.
