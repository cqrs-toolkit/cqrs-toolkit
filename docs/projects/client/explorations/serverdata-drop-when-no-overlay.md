...
Surfaced during 2026-05-07 review of [`§7.4`](../intent/requirements/0007-read-model-store.md#64-server-baseline-vs-effective-overlay-semantics) of `0007-read-model-store.md`. The intent is to keep `serverData` populated only when an overlay exists. Current implementation keeps it always populated for code simplicity (so that if an overlay arrives later, the baseline is already at hand). This was logged as a tolerated deviation, not the design preference.
...
Cross-cuts with [`§7.4`](../intent/requirements/0007-read-model-store.md#64-server-baseline-vs-effective-overlay-semantics) storage rules (under-intent vs current implementation) and the broader stateful redesign (which may also rework the baseline/overlay model).
